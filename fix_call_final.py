...
target2 = """                        val isEmergencyNumber = primaryContactPhone == "911" || primaryContactPhone == "112" || primaryContactPhone == "999"
                        when {
                            isEmergencyNumber -> {
                                val intent = Intent(Intent.ACTION_DIAL).apply { data = Uri.parse("tel:$primaryContactPhone") }
                                context.startActivity(intent)
...
replacement2 = """                        if (ContextCompat.checkSelfPermission(context, Manifest.permission.CALL_PHONE) == PackageManager.PERMISSION_GRANTED) {
                            val intent = Intent(Intent.ACTION_CALL).apply { data = Uri.parse("tel:$primaryContactPhone") }
                            context.startActivity(intent)
                        } else {
                            callPermissionLauncher.launch(Manifest.permission.CALL_PHONE)
...
print("Fixed EmergencyScreen call logic")
